...
# among all edges of a core function perturbation, using the randomization of GRN
...
# we load the random GRNs and recalculate the numbers with these random GRN
for (nn in 1:nRand){
...
save(file = 'randomization_result_edge_quantification_randGRN.Rdata',rand_stat)
hist(rand_stat$default_CR_full$rand_rate)
hist(rand_stat$default_CR_single$rand_rate)
pdf(paste('figures/full_randomization_results_randGRN.pdf',sep = ''),width = 7,height = 6)
hist(rand_stat$default_CR_full$rand_rate, 
...
write.csv(edge_quant,'figures/edge_quantification_randGRN.csv')
