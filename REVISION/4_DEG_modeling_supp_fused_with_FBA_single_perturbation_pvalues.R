...
scoreMat = read.csv('./../CR_model_final_run/output/delta_flux_scoreMat.csv',row.names = 1)
...
model_explained_long_obs = model_explained_long
...
rand_rate_down = c()
rand_rate2_down = c()
model_explained_long_rand = list()
...
for (nn in 1:nRand){
  ...
  model_explained_long_rand[[nn]] = model_explained_long
...
save(file = 'randomization_result_FBA_fused_model_single_perturbation.Rdata',model_explained_long_rand, rand_rate, rand_rate2, rand_rate_up, rand_rate2_up, rand_rate_down, rand_rate2_down)
hist(rand_rate)
hist(rand_rate2)
...
# calculate the empirical p-value for each perturbation 
pvalues = data.frame(cond = total_condition_analyzed)
pvalues$UP = NA
pvalues$DOWN = NA
pvalues$total = NA
for (i in 1:nrow(pvalues)){
  rand_vals_up = unlist(lapply(model_explained_long_rand, function(x){x$up[x$RNAi_cond == pvalues$cond[i] & x$variable == 'yes']}))
  rand_vals_down = unlist(lapply(model_explained_long_rand, function(x){x$down[x$RNAi_cond == pvalues$cond[i] & x$variable == 'yes']}))
  rand_vals_total = rand_vals_up + rand_vals_down
  
  obs_val_up = model_explained_long_obs$up[model_explained_long_obs$RNAi_cond == pvalues$cond[i] & model_explained_long_obs$variable == 'yes']
  obs_val_down = model_explained_long_obs$down[model_explained_long_obs$RNAi_cond == pvalues$cond[i] & model_explained_long_obs$variable == 'yes']
  obs_val_total = obs_val_up + obs_val_down
  
  pvalues$UP[i] = (1+sum(rand_vals_up >= obs_val_up))/(1+length(rand_vals_up))
  pvalues$DOWN[i] = (1+sum(rand_vals_down >= obs_val_down))/(1+length(rand_vals_down))
  pvalues$total[i] = (1+sum(rand_vals_total >= obs_val_total))/(1+length(rand_vals_total))

}

hist(-log10(pvalues$UP))
hist(-log10(pvalues$DOWN))
hist(-log10(pvalues$total))


# plot p-value agaist number of DEG 
pvalues$N_DEG = NA
for (i in 1:nrow(pvalues)){
  pvalues$N_DEG[i] = sum(model_explained_long_obs$up[model_explained_long_obs$RNAi_cond == pvalues$cond[i]]) + sum(model_explained_long_obs$down[model_explained_long_obs$RNAi_cond == pvalues$cond[i]])
}
pdf('figures/CR_model_single_perturbation_pvalue.pdf',height = 6, width = 6)
plot(log2(pvalues$N_DEG), -log10(pvalues$total), 
     xlab = 'log2(iCEL1314 DEG)', ylab = '-log10(CR model p-value)', pch = 16, col = 'black')
abline(v = log2(30), lty = 2, col = 'red')
# plot p-value by the number of associated core fucntions
# can also test the cleaned core function matrix
pvalues$N_core_functions = NA
for (i in 1:nrow(pvalues)){
  pvalues$N_core_functions[i] = sum(classMat[conditionInfo$RNAi_WBID[pvalues$cond[i] == str_replace(conditionInfo$RNAiID,' ','_')],] == 1)
}

boxplot(-log10(pvalues$total[pvalues$N_DEG>30]) ~ pvalues$N_core_functions[pvalues$N_DEG>30],
        ylab = '-log10(CR model p-value)', xlab = 'Number of core functions associated with the perturbation')
# this analysis is confounded because as the core function increases, more DEG will be just consider explained (like if all five core functions are associated with a perturbation (i.e., ETC genes), any up DEG is explained. So CR model is more meaningful when assessed at systems level)
dev.off()


# overall, this analysis is less interpretable because of the power issue caused by small sample size in most conditions
